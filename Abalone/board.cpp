...
void board::mapCells(cell* row,const char& r,const int& z){          // Changing the shape of the board to a Diamond
    int f=1;
    if(r>='A'&& r<='F'){                                             // Lower half of the board 'A' - 'F'
        f=1;
    }else if(r>'F'&& r<='K'){
        f=r-'F'+1;                                                   // Upper half of the board 'F' - 'K'
    }else return;
    for(int i=0;i<z;i++){
        stringstream ss;
        ss<<r;
...
board::board():woc(0),boc(0){
    char r = 'K';
    for(int i=0;i<11;i++){                                          // The number of rows is now 11 
        if(i<6){
            ...
            rows.push_back(new cell[6+5-i]);
            connectHorizontal(rows[i],6+5-i);
            mapCells(rows[i],r--,6+5-i);
            if(i>=6){
                setMarbles(rows[i],6+5-i,'@');
            }else
                setMarbles(rows[i],6+5-i,'+');
        }
    }
    for(int i=1;i<11;i++){
        if(i<6){                                                   // Our Diagonals are now 6
            connectDiagonal(rows[i],rows[i-1],0,1+i-1,true);          
            connectDiagonal(rows[i],rows[i-1],1,1+i-1,true);
        }
        else{
            connectDiagonal(rows[i-1],rows[i],0,6+5-i,false);
            connectDiagonal(rows[i-1],rows[i],1,6+5-i,false);
        }
    }
    cell* linep = rows[0];                                        //start at K6
    while(linep!=nullptr){
        cell* cellp = linep;
        while(cellp!=nullptr){
            for(int i=0;i<6;i++){
...
string board::traverseDiagonal() const{
    map<string,cell*>::const_iterator it = cells.find(string("F1"));
...
bool board::inPlay() const{
    if((woc<3) && (boc<3)) //LETS NOT FORGET
        return true;
    return false;
}
