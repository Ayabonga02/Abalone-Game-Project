...
movement humanAgent::move(movement const* const om,board bd){
    string l;
    int n,f,m,mtype;                                          
    bool scm = false, valid = false;
    string str="";
    
    cout<<"Enter your move in the following format(l n fd md)(i.e G4 2 5 3): "<<endl;
    cin>>l>>n>>f>>m;

    valid = bd.validateMove(symbol,l,n,f,m,mtype,scm);
    ...
    map<string, cell*> cells = bd.getCells();
    cell* cp = cells.at(l);
    int distance2edge = cp->getDistanceToEdge(m);
    return movement(l,n,f,m,distance2edge,mtype,scm);
